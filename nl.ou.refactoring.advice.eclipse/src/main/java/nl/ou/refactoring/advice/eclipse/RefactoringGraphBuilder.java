...
public final class RefactoringGraphBuilder {
	...
	/**
	 * Initialises a new instance of {@link RefactoringGraphBuilder}. Not used,
	...
	/**
	 * Appends code nodes from a {@link IJavaElement} to a Refactoring Advice Graph
	 * (RAG).
	 * 
	 * @param graph   The Refactoring Advice Graph (RAG).
	 * @param element The Java element from which to append code nodes to a
	 *                Refactoring Advice Graph (RAG).
	 * @return The Refactoring Advice Graph (RAG).
	 * @throws ArgumentNullException                    Thrown if element is null.
	 * @throws ElementResourceNotFoundException         Thrown if the element's
	 *                                                  resource could not be found.
	 * @throws ElementResourceLocationNotFoundException Thrown if the element's
	 *                                                  resource location could not
	 *                                                  be found.
	 * @throws FileNotFoundException                    Thrown if the element's file
	 *                                                  could not be found.
	 * @throws JavaModelException                       Thrown if determining a
	 *                                                  SourceType element's super
	 *                                                  class failed.
	 */
	public static final Graph append(Graph graph, IJavaElement element)
			throws ArgumentNullException, ElementResourceNotFoundException, ElementResourceLocationNotFoundException,
		...
		if (element instanceof IType) {
			LOGGER.info(
					...
					element.getElementName());
			graph =
					new EclipseResolutionProvider(element.getJavaProject())
			...
			if (methodClassNodeOptional.isEmpty()) {
				graph =
						new EclipseResolutionProvider(element.getJavaProject())
								.resolveByFullyQualifiedName(
										graph,
										methodClass.getFullyQualifiedName(),
										GraphNodeClass.class)
								.get()
								.getGraph();
			}
	...
	private static Graph readCallers(Graph graph, IMethod method) {
		...
		try {
			...
			final var searchScope =
					SearchEngine.createJavaSearchScope(new IJavaProject[] { methodProject }, IJavaSearchScope.SOURCES);
					...
					new SearchRequestor() {
						...
						public void acceptSearchMatch(SearchMatch match) {
							try {
								...
								if (callerJavaElementProject == null
										|| !methodProject.equals(callerJavaElementProject)) {
									return;
								}
								...
								final var callerJavaElementProjectRelativePath =
										callerJavaElementProjectResource.getProjectRelativePath().toString();
								if (callerJavaElementProjectRelativePath == null) {
									return;
								}

								final var methodProjectRelativePath =
										methodProject.getResource().getProjectRelativePath().toString();
								...
								case IMethod callerMethod: {
									final var callerMethodClassName =
											callerMethod.getDeclaringType().getFullyQualifiedName();
									final var callerMethodClassNodeOptional =
											graph.getNode(callerMethodClassName, GraphNodeClass.class);
									...
									if (callerMethodClassNodeOptional.isEmpty()) {
										...
										callerMethodClassNode =
												graph.getNode(callerMethodClassName, GraphNodeClass.class).get();
									} else {
										callerMethodClassNode = callerMethodClassNodeOptional.get();
									}
									final var callerMethodNode =
											callerMethodClassNode
													.getOperationNode(callerMethod.getElementName(), new ArrayList<>() // TODO
																														// parameters
									)
													.get();
									...
									for (final var callerMethodStatement : callerMethodStatements) {
										if (callerMethodStatement instanceof GraphNodeExpressionStatement) {
											final var expressionStatement =
													(GraphNodeExpressionStatement) callerMethodStatement;
											...
											if (statementExpression instanceof GraphNodeMethodInvocationExpression) {
												final var methodInvocationExpression =
														(GraphNodeMethodInvocationExpression) statementExpression;
												final var invokedOperationNode =
														methodInvocationExpression.getInvokedOperationNode();
											}
										}
									}
									break;
	...
	private static Graph readHierarchy(final Graph graph, IType type) throws JavaModelException, FileNotFoundException {
		final var typeFullyQualifiedName = type.getFullyQualifiedName();
		final var classNodeBaseOptional = graph.getNode(typeFullyQualifiedName, GraphNodeClass.class);
		final var classNodeBase = classNodeBaseOptional.orElseGet(() -> {
			try {
				new EclipseResolutionProvider(type.getJavaProject())
						.resolveByFullyQualifiedName(graph, typeFullyQualifiedName, GraphNodeClass.class);
		...
		for (final var typeSupertype : typeSupertypes) {
			...
			try {
				new EclipseResolutionProvider(typeSupertype.getJavaProject()).resolveByFullyQualifiedName(
						graph,
						typeSupertype.getFullyQualifiedName(),
						GraphNodeClass.class);
				if (typeSupertype.isClass()) {
					final var classNode =
							graph.getNode(typeSupertype.getFullyQualifiedName(), GraphNodeClass.class).get();
		...
		for (final var typeSubtype : typeSubtypes) {
			try {
				new EclipseResolutionProvider(typeSubtype.getJavaProject())
						.resolveByFullyQualifiedName(graph, typeSubtype.getFullyQualifiedName(), GraphNodeClass.class);
				if (typeSubtype.isClass()) {
					final var classNode =
							graph.getNode(typeSubtype.getFullyQualifiedName(), GraphNodeClass.class).get();
