...
public final class RefactoringAdviceView {
	...
	public void show() {
		try {
			...
			inputsComposite.addGraphChangedListener(event -> {
				...
				final var changedRefactoringAdviceHtml = renderRefactoringAdviceHtml(changedGraph);
	...
	private static void centerShell(Shell shell, Shell parentShell) {
		if (parentShell != null) {
			...
			shell.setLocation((Display.getDefault().getBounds().width - shell.getSize().x) / 2,
	...
	private static void appendClassDiagram(final Graph graph, final Element bodyElement) {
		try {
			...
			final var classDiagramPlantUmlWriter = new GraphPlantUmlClassDiagramWriter(
					classDiagramPlantUmlStringWriter);
			classDiagramPlantUmlWriter.write(graph);
			final var classDiagramSvgString = GraphPlantUmlSvgGenerator
					.generate(classDiagramPlantUmlStringWriter.toString());
			final var classDiagramSvgDocumentBuilder = DocumentBuilderFactory.newInstance().newDocumentBuilder();
			final var classDiagramSvgDocument = classDiagramSvgDocumentBuilder
					.parse(new InputSource(new StringReader(classDiagramSvgString)));
	...
	private static void appendAdviceText(Graph graph, final Element bodyElement) {
		try {
			...
			final var graphHtmlTextWriter = new GraphHtmlTextWriter(
					new GraphHtmlWriterSettings(new URI("eclipse-resource://nl.ou.refactoring.advice.eclipse/")),
					adviceTextSection, nlpGrammarProcessor);
