...
public final class NLPConcatenationProviderTestArgumentsProvider implements ArgumentsProvider {
	...
	private static Arguments createGraphMoveMethodDoubleDefinition() {
		...
		final var classBeta =
			graph
				.getNode("nl.ou.refactoring.moveMethod.doubleDefinition.Beta", GraphNodeClass.class)
				.get();
		final var classBetaFoo1OperationNode =
			classBeta
				.getOperationNode("foo", List.of())
				.get();
		...
		var firstId =
			methodAddedId.compareTo(methodOtherId) > 0
				? methodOtherId
				: methodAddedId;
		var secondId =
			methodAddedId.compareTo(methodOtherId) > 0
