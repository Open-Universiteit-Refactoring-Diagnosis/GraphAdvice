...
public final class GraphNodeMicrostepTests {
	...
	private static final Locale[] SUPPORTED_LOCALES = { Locale.of("nl", "NL"), Locale.of("en", "GB") };

	@ParameterizedTest
	@MethodSource("getCaptionsTestCases")
	@DisplayName("Should get localised captions for nodes")
	public void getCaptionsTest(Class<? extends GraphNodeMicrostep> microstepNodeClass) throws InstantiationException,
			IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException {
		final var graph = new Graph("Test");
		final var microstepNode = microstepNodeClass.getConstructors()[0].newInstance(graph);

		for (final var locale : SUPPORTED_LOCALES) {
			Locale.setDefault(locale);
			final var result = ((GraphNodeMicrostep) microstepNode).getCaption();
