...
public final class Tokens {
	...
	public final class Determiners {
		...
		/**
		 * The token for Possessive Pronouns. Declension is left to grammar and
		 * language.
		 */
		 ...
		 */
		public static Set<Long> all() {
			return Stream.concat(Articles.all().stream(), getAll(Determiners.class).stream())
	...
	public final class Nouns {
		...
		public final class Common {
			...
			private static final TokenStore<CommonNoun> COMMON_NOUNS = new TokenStore<>();

			/**
			 * <a href="https://en.wiktionary.org/wiki/class">Class</a>: A set of objects
			 * having the same behavior (but typically differing in state), or a template
			 * defining such a set in terms of its common properties, functions, etc.
			 */
			public static final long CLASS_OO_PROGRAMMING = "[common noun] class (OO programming)".hashCode();

			/**
			 * <a href="https://en.wiktionary.org/wiki/conflict">Conflict</a>: 2. An
			 * incompatibility, as of two things that cannot be simultaneously fulfilled.
			 */
			public static final long CONFLICT_INCOMPATIBILITY = "[common noun] conflict".hashCode();

			/**
			 * <a href="https://en.wiktionary.org/wiki/field">Field</a>: A physical or
			 * virtual location for the input of information in the form of symbols.
			 */
			public static final long FIELD_OO_PROGRAMMING = "[common noun] field (OO programming)".hashCode();

			/**
			 * <a href="https://en.wiktionary.org/wiki/method">Method</a>: A process by
			 * which a task is completed; a way of doing something (followed by the
			 * adposition of, to or for before the purpose of the process).
			 */
			public static final long METHOD = "[common noun] method".hashCode();

			/**
			 * <a href="https://en.wiktionary.org/wiki/microstep">Microstep</a>: A very
			 * small step (typically made by a stepping motor).
			 */
			...
			static {
				...
				COMMON_NOUNS.computeIfAbsent(
						FIELD_OO_PROGRAMMING,
						new CommonNoun(FIELD_OO_PROGRAMMING, SemanticClassification.ABSTRACT, Countability.COUNTABLE));
				COMMON_NOUNS.computeIfAbsent(
						METHOD,
			 ...
			 * @return The Common Noun wrapped in {@link Optional}, otherwise an empty
			 *         {@link Optional}.
			 */
		...
		/**
		 * Tokens for Verbal Nouns (verbs used as nouns).
		 */
		public final class Verbal {
			private Verbal() {
			}

			private static final TokenStore<CommonNoun> VERBAL_NOUNS = new TokenStore<>();
			...
			/**
			 * <a href="https://en.wiktionary.org/wiki/renaming">Renaming</a>: An act in
			 * which something is renamed.
			 */
			public static final long RENAMING = "[verbal noun] renaming".hashCode();

			static {
				VERBAL_NOUNS.computeIfAbsent(
						REFACTORING,
						new CommonNoun(REFACTORING, SemanticClassification.ABSTRACT, Countability.COUNTABLE));
				VERBAL_NOUNS.computeIfAbsent(
						RENAMING,
						new CommonNoun(RENAMING, SemanticClassification.ABSTRACT, Countability.COUNTABLE));
			}

			/**
			 * Gets all verbal noun tokens.
			 * 
			 * @return An unmodifiable set of verbal noun tokens.
			 */
			public static Set<Long> all() {
				return getAll(Verbal.class);
			}

			/**
			 * Gets the Verbal Noun with the specified token.
			 * 
			 * @param token The token of the Verbal Noun.
			 * @return The Verbal Noun wrapped in {@link Optional}, otherwise an empty
			 *         {@link Optional}.
			 */
			public static Optional<CommonNoun> get(long token) {
				return VERBAL_NOUNS.get(token);
			}
		}
		 ...
		 */
		public static Set<Long> all() {
			return Stream.concat(Common.all().stream(), getAll(Nouns.class).stream())
					.collect(Collectors.toUnmodifiableSet());
		}
	}
	...
	public final class Prepositions {
		...
		/**
		 * <a href="https://en.wiktionary.org/wiki/from">from</a>: Indicating removal or
		 * separation.
		 */
		public static final long FROM_REMOVAL_SEPARATION = "[preposition] from (removal or separation)".hashCode();

		/**
		 * <a href="https://en.wiktionary.org/wiki/in">in</a>: Used to indicate
		 * location, inclusion, or position within spatial, temporal or abstract limits.
		 */
		public static final long IN = "[preposition] in".hashCode();

		/**
		 * <a href="https://en.wiktionary.org/wiki/of">of</a>: 4. Expressing agency. 2.
		 * Used to introduce the "subjective genitive"; following a noun to form the
		 * head of a postmodifying noun phrase.
		 */
		public static final long OF_AGENCY_SUBJECTIVE_GENITIVE = "[preposition] of".hashCode();

		/**
		 * <a href="https://en.wiktionary.org/wiki/to">to</a>: 3. Used to indicate the
		 * target or recipient of an action.
		 */
		public static final long TO_TARGET_RECIPIENT = "[preposition] to (target or recipient)".hashCode();

		/**
		 * <a href="https://en.wiktionary.org/wiki/with">with</a>: 1. Against.
		 */
		public static final long WITH_AGAINST = "[preposition] with (against)".hashCode();
		...
		static {
			...
			PREPOSITIONS.computeIfAbsent(WITH_AGAINST, new Preposition(WITH_AGAINST));
		}
		 ...
		 * @return The Preposition wrapped in an {@link Optional}, otherwise an empty
		 *         {@link Optional}.
		 */
	...
	public final class Verbs {
		...
		public final class Auxiliary {
			 ...
			 * @return The Auxiliary Verb that is linked to the specified token, wrapped in
			 *         an {@link Optional}, otherwise an empty {@link Optional}.
			 */
			public static Optional<AuxiliaryVerb> get(long token, Verb mainVerb) throws ArgumentNullException {
		...
		public final class Lexical {
			 ...
			 * To Conflict.
			 */
			public static final long CONFLICT = "[lexical verb] to conflict".hashCode();
			...
			static {
				...
				LEXICAL_VERBS
						.computeIfAbsent(CAUSE, phrase -> new LexicalVerb(CAUSE, VerbTransitivity.TRANSITIVE, phrase));
				LEXICAL_VERBS.computeIfAbsent(
						CONFLICT,
						phrase -> new LexicalVerb(CONFLICT, VerbTransitivity.INTRANSITIVE, phrase));
				LEXICAL_VERBS
						.computeIfAbsent(HAVE, phrase -> new LexicalVerb(HAVE, VerbTransitivity.TRANSITIVE, phrase));
				LEXICAL_VERBS.computeIfAbsent(
						REFACTOR,
						phrase -> new LexicalVerb(REFACTOR, VerbTransitivity.TRANSITIVE, phrase));
				LEXICAL_VERBS.computeIfAbsent(
						REMOVE,
						phrase -> new LexicalVerb(REMOVE, VerbTransitivity.TRANSITIVE, phrase));
			}
			...
			/**
			 * Gets a factory function for the {@link LexicalVerb} associated with the
			 * specified token.
			 * 
			 * @param token The token that represents a {@link LexicalVerb}.
			 * @return If found, the requested factory function for {@link LexicalVerb}
		...
		public static Set<Long> all() {
			...
			return tokensSet.stream().collect(Collectors.toUnmodifiableSet());
		}
	}
	...
	private static Set<Long> getAll(Class<?> classType) {
		return Set.of(classType.getDeclaredFields()).stream().filter(f -> {
